...
########PART2#########
def Checkhori2(cas,prevres,pos):
	i=1
	tmp=cas[::]
	while i<len(tmp)/2:
		tmp2=tmp[i:i*2]
	...
	while i<len(tmp)/2:
		...
		i+=1
	return 0
def Checkverti2(cas,prevres,pos):
	tmp=[]
	st=""
	for colonne in range(len(cas[0])):
		for ligne in range(len(cas)):
...
def Comptagebourrin(licas):#mettre un n
	...
	for cas in licas:
		...
		for i in range(len(cas)):
			prevresh=Checkhori(cas)
			prevresv=Checkverti(cas)
			for i2 in range(len(cas[0])):
				if cas[i][i2]=="#":
					tmp[i]=cas[i][0:i2]+"."+cas[i][i2+1:len(cas[i])]
				else:
					tmp[i]=cas[i][0:i2]+"#"+cas[i][i2+1:len(cas[i])]
				t=Checkhori2(tmp,prevresh,i)			
				if t==0:
					t=Checkverti2(tmp,prevresv,i2)
					if t!=0 :
						som+=t
						run=False
						break	
